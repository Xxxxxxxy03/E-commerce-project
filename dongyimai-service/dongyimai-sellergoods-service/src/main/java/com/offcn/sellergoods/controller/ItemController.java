...
public class ItemController {
    ...
    //根据指定状态获取对于sku数据集合
    @ApiOperation(value = "根据指定状态获取对于sku数据集合",notes = "根据指定状态获取对于sku数据集合",tags = {"ItemController"})
    @ApiImplicitParam(name = "status",value = "状态",required = true,type = "path")
    ...
    public Result<List<Item>> findByStatus(@PathVariable("status") String status){
        ...
        return new Result<>(true,StatusCode.OK,"查询sku数据成功",itemList);
    }
    ...
    public Result decrCount(@RequestParam("username") String username){
        try {
            ...
            e.printStackTrace();
            return new Result(false,StatusCode.ERROR,"减少库存失败！");
        }
    }
}
